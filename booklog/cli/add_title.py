from __future__ import annotations

import html
from collections.abc import Callable
from dataclasses import dataclass, field
from typing import Literal

from prompt_toolkit.shortcuts import confirm

from booklog.cli import ask, radio_list, select_author, select_title
from booklog.cli.utils.array_to_sentence import array_to_sentence
from booklog.repository import api as repository_api
from booklog.repository.types import NonEmptyList

Stages = Literal[
    "ask_for_authors",
    "ask_for_kind",
    "ask_for_title",
    "ask_for_subtitle",
    "ask_for_year",
    "ask_for_included_titles",
    "persist_title",
    "end",
]


@dataclass(kw_only=True)
class State:
    stage: Stages = "ask_for_authors"
    kind: repository_api.Kind | None = None
    title: str | None = None
    title_authors: list[repository_api.TitleAuthor] = field(default_factory=list)
    subtitle: str | None = None
    year_published: str | None = None
    included_titles: list[str] = field(default_factory=list)


def prompt() -> None:
    state = State()

    state_machine: dict[Stages, Callable[[State], State]] = {
        "ask_for_authors": ask_for_authors,
        "ask_for_kind": ask_for_kind,
        "ask_for_title": ask_for_title,
        "ask_for_subtitle": ask_for_subtitle,
        "ask_for_year": ask_for_year,
        "ask_for_included_titles": ask_for_included_titles,
        "persist_title": persist_title,
    }

    while state.stage != "end":
        state_machine[state.stage](state)


def persist_title(state: State) -> State:
    assert state.year_published
    assert state.title
    assert state.kind
    assert state.title_authors

    repository_api.create_title(
        title=state.title,
        title_authors=NonEmptyList.from_sequence(state.title_authors),
        subtitle=state.subtitle,
        year=state.year_published,
        kind=state.kind,
        included_title_ids=state.included_titles,
    )

    author_names = array_to_sentence([author.author().name for author in state.title_authors])

    if confirm(f"Add more titles by {author_names}?"):
        state.stage = "ask_for_kind"
    else:
        state.stage = "end"

    return state


def ask_for_year(state: State) -> State:
    state.year_published = None

    year = ask.prompt("Year First Published: ")

    if not year:
        state.stage = "ask_for_subtitle"
        return state

    state.year_published = year
    state.stage = "ask_for_included_titles"
    return state


def ask_for_authors(state: State) -> State:
    state.title_authors = []

    while True:
        author = select_author.prompt()

        if not author:
            state.stage = "end"
            return state

        author_notes = ask.prompt("Notes: ")

        if author_notes is None:
            continue

        if author_notes == "":
            author_notes = None

        state.title_authors.append(
            repository_api.TitleAuthor(notes=author_notes, author_slug=author.slug)
        )

        if not confirm("Add more Authors?"):
            state.stage = "ask_for_kind"
            return state


def ask_for_included_titles(state: State) -> State:
    state.included_titles = []

    if state.kind not in {"Collection", "Anthology"}:
        state.stage = "persist_title"
        return state

    while True:
        title = select_title.prompt()

        if not title and not state.included_titles:
            state.stage = "ask_for_year"
            return state

        if title:
            state.included_titles.append(title.slug)

        if confirm("Add more?"):
            continue

        state.stage = "persist_title"
        return state


def ask_for_title(state: State) -> State:
    state.title = None
    title = ask.prompt("Title: ")

    if not title:
        state.stage = "ask_for_kind"
        return state

    if confirm(title):
        state.title = title
        state.stage = "ask_for_subtitle"

    return state


def ask_for_subtitle(state: State) -> State:
    state.subtitle = None

    if state.kind != "Nonfiction":
        state.stage = "ask_for_year"
        return state

    subtitle = ask.prompt("Sub-title: ")

    if subtitle is None:
        state.stage = "ask_for_title"
        return state

    if confirm(subtitle):
        state.subtitle = subtitle or None
        state.stage = "ask_for_year"

    return state


def ask_for_kind(state: State) -> State:
    state.kind = None

    kind = radio_list.prompt(
        title="Select kind:",
        options=[
            (kind, f"<cyan>{html.escape(kind)}</cyan>")
            for kind in sorted(repository_api.TITLE_KINDS)
        ],
    )

    if not kind:
        state.stage = "ask_for_authors"
        return state

    state.kind = kind
    state.stage = "ask_for_title"

    return state
